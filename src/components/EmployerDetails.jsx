...
const EmployerDetails = () => {
  ...
  const validateBasicInformationTab = () => {
  const newErrors={};
  ...
  if(!formData.employerId){
    newErrors.employerId = "EmployerId is required";
  }
...
const validateForm = () =>{
  return validateBasicInformationTab();
}
...
const handleInputChange = (field, value) => {
   ...
   setValidateError((prevErrors)=>{
    ...
    switch(field){
      case "employerName":
        if(value.trim() !== "") delete newErrors.employerName;
        break;
      case "employerId":  
      if(value.trim() !== "") delete newErrors.employerId;
      break;
      case "employerEmail":
        if(value.trim() !== "") delete newErrors.employerEmail;
        break;
      default:
        break;  
    }
  ...
  const handleSubmit = async (e) => {
    ...
    const  isBasicDetailsValid = validateBasicInformationTab();
    if(!isFormValid ||!isBasicDetailsValid || Object.keys(validateError).length > 0) {
...
setSubmitError("")
        ...
        // } else{
        //   setSubmitError("Something went wrong while updating employer.")
        // }
      } finally {
        setLoading(false);
      }
 ...
 const renderBasicInformation = () => (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700">
            Employer ID <span className="text-red-600">*</span>
          </label>
          <input
            type="text"
            className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm px-3 py-2 border"
              ...
              <div className="shadow sm:rounded-md sm:overflow-hidden">
                ...
                {Object.values(validateError).length > 0 && (
                  <div className="bg-red-100 text-red-800 p-2 rounded mb-4">
                    {Object.entries(validateError).map(([key,message],i)=>(
