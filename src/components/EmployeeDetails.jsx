...
const EmployeeDetails = () => {
  ...
  const fetchEmployees = async () => {
    try {
      ...
      switch (selectedTab) {
        case "active":
          url = "http://localhost:8081/api/employee-details/fetch/active-employees";
          break;
        case "leaving":
          url = "http://localhost:8081/api/employee-details/fetch/Ready-for-leave-employees";
          break;
        case "inactive":
          url = "http://localhost:8081/api/employee-details/fetch/inactive-employees";
          break;
        case "all":
          url = "http://localhost:8081/api/employee-details/allEmployees";  
          break;
        default:
          url = "http://localhost:8081/api/employee-details/fetch/active-employees";
      } 
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete) {
      try {
        ...
        await axios.delete(`http://localhost:8081/api/employee-details/delete/${id}`)
...
if (dobDate >= today) {
 ...
 setErrors(newErrors);
  ...
  const handleUpdate = async (e) => {
    ...
    const personalDetailsValidation = validatePersonalDetailsTab();

    const isValid = payValidation.isValid && taxValidation.isValid && personalDetailsValidation.isValid;
    const allWarnings = [...taxValidation.warnings];
    const allErrors = { ...payValidation.errors, ...taxValidation.errors, ...personalDetailsValidation.errors };

    console.log("payValidation:", payValidation);
console.log("taxValidation:", taxValidation);
console.log("personalDetailsValidation:", personalDetailsValidation);
...
console.log("allWarnings:", allWarnings);
    ...
    try {
      ...
      if (p45Form || starterChecklist) {
        const editDataUpload = new editData();

        if (p45Form) editDataUpload.append("p45Document", p45Form);
        if (starterChecklist) editDataUpload.append("starterChecklist", starterChecklist);

        try {
          const fileData = await axios.post(
            "http://localhost:8081/api/employee-details/upload-documents",
            editDataUpload,
            {
      ...
      const response = await axios.put(
        `http://localhost:8081/api/employee-details/update/${selectedEmployee.id}`,
        updatedData,
        {
          headers: {
